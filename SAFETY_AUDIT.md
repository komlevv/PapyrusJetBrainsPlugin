...
Failure is non-destructive on the IDE side: the last successful project snapshot and import library stay available, while the Projects tab surfaces the validation/server error. Created/deleted PSC events are routed through the same validation gate rather than directly invoking upstream watched-file reload. No timer-based recovery is used. If an explicitly requested reload remains unconfirmed, a second explicit Refresh first revalidates the current PPJs and then restarts only the Papyrus LSP through the public `LspClientManager.stopAndRestartClientsIfNeeded(...)` lifecycle; no restart is triggered by elapsed wall-clock time.
...
0.2.151 keeps the 0.2.150 Papyrus-only editor-local shortcut lifetime and does not mutate the global keymap. The only behavioral change is replacing the non-navigating `ActionUtil.wrap(...)` delegate with a local `DumbAwareAction` that re-executes the already-registered platform `GotoDeclaration` action through public `ActionManager.tryToExecute(...)`. It passes the current editor content component as context, no input event, no keyboard-specific action place, and `now=false`. In Platform 262 that public asynchronous branch waits for focus, rebuilds the component `DataContext`, updates the supplied action without `runUpdateSessionForInputEvent(...)`, and then performs it. No Rider internal API, remote action API, file write, process launch, PPJ/import change, or LSP protocol change is introduced.

## 0.2.150 shortcut-routing safety review

0.2.150 changes only editor-local action routing for Papyrus Go to Declaration. It does not write files, mutate the global keymap, start processes, change LSP requests, alter PPJ/import resolution, or use JetBrains internal/experimental remote-action APIs. The installer subscribes through public `EditorFactoryListener`, installs the action only on `.psc` editor content components, copies the registered platform `GotoDeclaration` `ShortcutSet` rather than hard-coding Ctrl+B, and stores the local action in editor user data and removes the registration explicitly from `EditorFactoryListener.editorReleased(...)` with public `AnAction.unregisterCustomShortcutSet(...)`. The local action is the public `ActionUtil.wrap(...)` wrapper around the registered platform `GotoDeclaration` action. This preserves the delegate's update behavior, dumb-awareness, injected-context behavior, update thread, and `PerformWithDocumentsCommitted` contract rather than reimplementing them.

The change remains pending the real Windows 59/20/45 gate; no verified-status claim is made until all three physical shortcut tests pass.
